...
export default class WebServer {
  ...
  public startServer(): void {
    ...
    app.use("/", express.static(path.resolve("/../static_html")));
    ...
    app.get("/print/:songId", async (req: any, res: any) => {
      res.setHeader("Content-Type", "application/json");
      try {
        const song = this.songsLib.getSongById(req.params.songId);
    ...
    app.get("/search/:term", async (req: any, res: any) => {
      ...
      res.send(
        JSON.stringify(this.songsLib.findSongByString(req.params.term, opts))
      );
    });
    ...
    app.listen(this.PORT, () => {
      console.log(
        `Rest Endpoint Server started listening on port ${this.PORT}`
      );
    });
  }
}
